...
class UNet(nn.Module):
    def __init__(self, in_channel=1, class_num=2, useBN=False, drop_rate=0):
        ...
        # down convolutions
        self.conv1 = conv3x3(in_channel, 64, useBN=useBN,
        ...
        self.conv0 = nn.Sequential(nn.ReflectionPad2d(1),
                                   ...
                                   nn.ReLU(),
                                   # nn.InstanceNorm2d(num_features=2) #viens d etre rajoute
                                   )
    ...
    # according to the original paper of UNet
    ...
    def forward(self, x):
        ...
        # there was several mistakes that I fixed...
        conv5m_out = torch.cat((self.upsample54(output5), output4), 1)
        conv4m_out = self.conv4m(conv5m_out)
        # conv4m_out replaced output4
        conv4m_out = torch.cat((self.upsample43(conv4m_out), output3), 1)
        conv3m_out = self.conv3m(conv4m_out)

        # conv3m_out replaced output3
        conv3m_out = torch.cat((self.upsample32(conv3m_out), output2), 1)
        conv2m_out = self.conv2m(conv3m_out)

        # conv2m_out replaced output2
        conv2m_out = torch.cat((self.upsample21(conv2m_out), output1), 1)
        conv1m_out = self.conv1m(conv2m_out)

        final = self.conv0(conv1m_out)

        # this final layer is different from the original paper (I need to check what this softmax layer is useful for)
...
def test():
    ...
    # (n_samples, input_channels, size, size)
